;(function () {
	...
	var activitiesCarousel = function(){
		var owl = $('.owl-carousel-activities');
		owl.owlCarousel({
			items: 4,
			loop: true,
			margin: 10,
    ...
    var acervoAnimate = function() {
        var acervo = $('#fh5co-acervo');
        if ( acervo.length > 0 ) { 
            acervo.waypoint( function( direction ) {
                if( direction === 'down' && !$(this.element).hasClass('animated') ) {
                    setTimeout(function() {
                        acervo.find('.to-animate').each(function( k ) {
                            var el = $(this);
                            setTimeout ( function () {
                                el.addClass('fadeInUp animated');
                            },  k * 200, 'easeInOutExpo' );
	...
	var magnificPopupInit = function() {
		$('.image-popup').magnificPopup({
			...
			gallery: {
				...
				tNext: 'Próximo (Seta para a direita)',
